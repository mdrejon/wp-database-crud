...
class WTDDB_ADMIN {
	...
	public function wtddb_add_new_data() {
		// Check nonce.
		if ( ! isset( $_POST['ajax_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ajax_nonce'] ) ), 'wtddb_nonce' ) ) {
	...
	public function wtddb_edit_data() {
		...
		$form_data = $_POST['formData'];
		parse_str( $form_data, $form_fields );
		$id    = sanitize_text_field( $form_fields['id'] );
		$name  = sanitize_text_field( $form_fields['name'] );
		$email = sanitize_email( $form_fields['email'] );
				...
				<div class="wtddb-add-new-popup-content">
					...
					<form class="wtddb-addnew-form" action="" method="post">
						<div class="wtddb-form-group">
							...
							<input type="text" name="name" placeholder="Enter The Name" id="name" required>
						</div>
						<div class="wtddb-form-group">
			...
			<div class="wtddb-edit-popup ">
				<div class="wtddb-add-new-popup-content">
					...
					<form class="wtddb-edit-form" action="" method="post">
						...
						<div class="wtddb-form-group"> 
							<button type="submit" class="button button-primary wtddb-edit-form-submit"><?php echo esc_html( __( 'Update Data', 'wp-database-crud' ) ); ?></button>
						</div>
					</form>
				</div>
			</div>	
	...
	public function wtddb_admin_page_content() {
		...
		if ( 0 === count( $results ) ) {
			echo '<div class="wtddb-emptydata ">
				<p>' . esc_html( __( 'No Data Found', 'wp-database-crud' ) ) . '</p>
			</div>';
			return;
		}
